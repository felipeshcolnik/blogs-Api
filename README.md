...
# Sobre o projeto BlogsAPI!

Nesse projeto, foi construido um back-end usando `ORM` com o pacote `sequelize` do `npm`, para criar e associar tabelas utilizando `models` do `sequelize`.
 Para incluir as novas informações no banco de dados, criei uma conexão através de endpoints utilizando API RESTFULL.
...
Arquiteturei, desenvolvi uma API de um CRUD posts de blog (com o sequelize). Começando pela API, você vai desenvolver alguns endpoints (seguindo os princípios do REST) que estarão conectados ao seu banco de dados. Lembre-se de aplicar os princípios SOLID!
...
2. Instale as dependências do projeto digitando:
```npm install```
...
### 1 - Esta aplicação tem o endpoint POST `/user`

#### Com as seguintes funcionalidades:

- O endpoint é capaz de adicionar um novo user a sua tabela no banco de dados;
...
- O corpo da requisição tem o seguinte formato:
  ...
  ```
- O campo `displayName` só será aceito for uma string com no mínimo de 8 caracteres;

- O campo `email` será considerado válido se tiver o formato `<prefixo>@<domínio>` e se for único. Este campo é obrigatório.

- A senha deverá conter 6 caracteres. Ela é obrigatória.

- Caso exista uma pessoa com o mesmo email na base, o endpoint retorna o seguinte erro:
...
### 3 - A aplicação tem um endpoint GET `/user`
...
### 4 - A aplicação tem um endpoint GET `/user/:id`
...
### 5 - A aplicação tem o endpoint POST `/categories`
...
### 6 - A aplicação tem o endpoint GET `/categories`
...
### 7 - A aplicação tem o endpoint POST `/post`
...
### 8 - A aplicação tem o endpoint GET `/post`
...
### 9 - A aplicação tem o endpoint GET `post/:id`
...
### 10 - A aplicação tem o endpoint PUT `/post/:id`
...
### 11 - A aplicação tem o endpoint DELETE `post/:id`
...
### 12 - A aplicação tem o endpoint DELETE `/user/me`
...
### 13 - A aplicação tem o endpoint GET `post/search?q=:searchTerm`
